...
export function setupOutputFolder(fpath: string, quorumConfig: QuorumConfig): string {
  ...
  [GETH_SUB, BESU_SUB].forEach((client) => {
...
export function createStaticNodes(path: string, nodes: string[], besu: boolean, goq: boolean): void {
  ...
  if (goq === true) {
    fs.writeFileSync(path + GETH_SUB + "/static-nodes.json", JSON.stringify(staticNodesGoQ, null, 2));
  }
}
...
export function createGoQuorumPermissionsFile(path: string, nodes: string[]): void {
  ...
  fs.writeFileSync(path + GETH_SUB + "/permissioned-nodes.json", JSON.stringify(staticNodes, null, 2));
  fs.writeFileSync(path + GETH_SUB + "/disallowed-nodes.json", JSON.stringify([], null, 2));
}
