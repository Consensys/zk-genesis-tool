...
export async function generateNetworkConfig(quorumConfig: QuorumConfig): Promise<string> {
  ...
  switch (consensus) {
    case Consensus.clique: {
      ...
      genesis.createGethGenesis(outputDir, quorumConfig, quorumExtraData, NodeKeyConcat);
    ...
    case Consensus.raft: {
      ...
      genesis.createGethGenesis(outputDir, quorumConfig, extraData, NodeKeyConcat);
      fileHandler.createGoQuorumPermissionsFile(outputDir, allNodesEnodes);
      break;
    }
    case Consensus.ibft: {
